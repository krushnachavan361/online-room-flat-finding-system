<?php
include("../includes/owner_auth.php");
include("../includes/db.php");

$user_id = $_SESSION['user_id'];

// OWNER DETAILS
$user = mysqli_fetch_assoc(mysqli_query($conn,
"SELECT * FROM users WHERE id='$user_id'"));

// TOTAL ROOMS
$rooms = mysqli_num_rows(mysqli_query($conn,
"SELECT * FROM rooms WHERE user_id='$user_id'"));

// TOTAL BOOKINGS
$bookings = mysqli_num_rows(mysqli_query($conn,
"SELECT * FROM bookings WHERE owner_id='$user_id'"));

// PENDING BOOKINGS
$pending = mysqli_num_rows(mysqli_query($conn,
"SELECT * FROM bookings 
WHERE owner_id='$user_id'
AND status='pending'"));

// ACCEPTED BOOKINGS
$accepted = mysqli_num_rows(mysqli_query($conn,
"SELECT * FROM bookings 
WHERE owner_id='$user_id'
AND status='accepted'"));

// TOTAL MESSAGES
$messages = mysqli_num_rows(mysqli_query($conn,
"SELECT * FROM messages
WHERE receiver_id='$user_id'"));

// RECENT BOOKINGS
$recent = mysqli_query($conn,
"SELECT b.*, u.name, r.room_title
FROM bookings b
JOIN users u ON b.user_id=u.id
JOIN rooms r ON b.room_id=r.id
WHERE b.owner_id='$user_id'
ORDER BY b.created_at DESC
LIMIT 5");

// ESTIMATED EARNINGS
$earnings = $accepted * 5000;
?>

<!DOCTYPE html>
<html>
<head>
<title>Commercial Owner Dashboard</title>

<!-- FONT -->
<link href="https://fonts.googleapis.com/css2?family=Poppins&display=swap" rel="stylesheet">

<!-- ICONS -->
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">

<!-- CHART -->
<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>

<style>

*{
    margin:0;
    padding:0;
    box-sizing:border-box;
}

body{
    font-family:'Poppins',sans-serif;
    background:#f4f6f9;
    display:flex;
    transition:0.3s;
}

/* DARK MODE */
body.dark{
    background:#111827;
    color:white;
}

/* SIDEBAR */
.sidebar{
    width:250px;
    height:100vh;
    background:#111827;
    color:white;
    position:fixed;
    padding-top:20px;
}

.logo{
    text-align:center;
    font-size:24px;
    margin-bottom:30px;
    color:#00ffcc;
    font-weight:bold;
}

.sidebar a{
    display:block;
    color:white;
    padding:15px 25px;
    text-decoration:none;
    transition:0.3s;
}

.sidebar a:hover{
    background:#1f2937;
    color:#00ffcc;
}

/* MAIN */
.main{
    margin-left:250px;
    width:100%;
    padding:20px;
}

/* TOPBAR */
.topbar{
    display:flex;
    justify-content:space-between;
    align-items:center;
    margin-bottom:25px;
}

/* TOP RIGHT */
.top-right{
    display:flex;
    align-items:center;
    gap:20px;
}

/* NOTIFICATION */
.notify{
    position:relative;
    font-size:24px;
}

.badge{
    position:absolute;
    top:-8px;
    right:-10px;
    background:red;
    color:white;
    border-radius:50%;
    padding:3px 7px;
    font-size:12px;
}

/* DARK BUTTON */
.dark-btn{
    padding:8px 12px;
    border:none;
    border-radius:8px;
    cursor:pointer;
    background:#111827;
    color:white;
}

/* SEARCH */
.search input{
    padding:10px;
    border-radius:10px;
    border:1px solid #ccc;
    width:250px;
}

/* WELCOME */
.welcome{
    background:linear-gradient(135deg,#667eea,#764ba2);
    color:white;
    padding:20px;
    border-radius:15px;
    margin-bottom:25px;
}

/* CARDS */
.cards{
    display:flex;
    flex-wrap:wrap;
    gap:20px;
}

.card{
    flex:1 1 220px;
    background:white;
    border-radius:15px;
    padding:25px;
    box-shadow:0 5px 15px rgba(0,0,0,0.1);
    transition:0.3s;
}

.dark .card{
    background:#1f2937;
    color:white;
}

.card:hover{
    transform:translateY(-5px);
}

.card i{
    font-size:30px;
    color:#00b894;
    margin-bottom:10px;
}

.count{
    font-size:28px;
    font-weight:bold;
}

/* BOXES */
.chart-box,
.table-box{
    margin-top:30px;
    background:white;
    padding:20px;
    border-radius:15px;
    box-shadow:0 5px 15px rgba(0,0,0,0.1);
}

.dark .chart-box,
.dark .table-box{
    background:#1f2937;
}

/* TABLE */
table{
    width:100%;
    border-collapse:collapse;
}

table th,
table td{
    padding:12px;
    border-bottom:1px solid #ddd;
    text-align:center;
}

table th{
    background:#111827;
    color:white;
}

.pending{
    color:orange;
}

.accepted{
    color:green;
}

.rejected{
    color:red;
}

</style>
</head>

<body>

<!-- SIDEBAR -->
<div class="sidebar">

    <div class="logo">Owner Panel</div>

    <a href="dashboard.php">
        <i class="fas fa-chart-line"></i> Dashboard
    </a>

    <a href="../rooms/add_room.php">
        <i class="fas fa-plus"></i> Add Room
    </a>

    <a href="../rooms/view_rooms.php">
        <i class="fas fa-home"></i> My Rooms
    </a>

    <a href="bookings.php">
        <i class="fas fa-calendar-check"></i> Bookings
    </a>

    <a href="../messages/inbox.php">
        <i class="fas fa-comments"></i> Messages
    </a>

    <a href="profile.php">
        <i class="fas fa-user"></i> Profile
    </a>

    <a href="../logout.php">
        <i class="fas fa-sign-out-alt"></i> Logout
    </a>

</div>

<!-- MAIN -->
<div class="main">

    <!-- TOPBAR -->
    <div class="topbar">

        <h2>🏠 Commercial Dashboard</h2>

        <div class="top-right">

            <!-- SEARCH -->
            <div class="search">
                <input type="text" placeholder="Search...">
            </div>

            <!-- DARK MODE -->
            <button class="dark-btn" onclick="toggleDark()">
                🌙
            </button>

            <!-- NOTIFICATION -->
            <div class="notify">

                <i class="fas fa-bell"></i>

                <span class="badge">
                    <?php echo $pending; ?>
                </span>

            </div>

        </div>

    </div>

    <!-- WELCOME -->
    <div class="welcome">
        <h2>Welcome, <?php echo $user['name']; ?> 👋</h2>
        <p>Manage rooms professionally with analytics.</p>
    </div>

    <!-- CARDS -->
    <div class="cards">

        <div class="card">
            <i class="fas fa-home"></i>
            <h3>Rooms</h3>
            <div class="count"><?php echo $rooms; ?></div>
        </div>

        <div class="card">
            <i class="fas fa-calendar-check"></i>
            <h3>Bookings</h3>
            <div class="count"><?php echo $bookings; ?></div>
        </div>

        <div class="card">
            <i class="fas fa-clock"></i>
            <h3>Pending</h3>
            <div class="count"><?php echo $pending; ?></div>
        </div>

        <div class="card">
            <i class="fas fa-check-circle"></i>
            <h3>Accepted</h3>
            <div class="count"><?php echo $accepted; ?></div>
        </div>

        <div class="card">
            <i class="fas fa-comments"></i>
            <h3>Messages</h3>
            <div class="count"><?php echo $messages; ?></div>
        </div>

        <div class="card">
            <i class="fas fa-wallet"></i>
            <h3>Earnings</h3>
            <div class="count">₹<?php echo $earnings; ?></div>
        </div>

    </div>

    <!-- CHART -->
    <div class="chart-box">

        <h3>📊 Analytics</h3>

        <canvas id="chart"></canvas>

    </div>

    <!-- RECENT BOOKINGS -->
    <div class="table-box">

        <h3>📥 Recent Bookings</h3>

        <table>

            <tr>
                <th>User</th>
                <th>Room</th>
                <th>Status</th>
            </tr>

            <?php while($row=mysqli_fetch_assoc($recent)) { ?>

            <tr>

                <td><?php echo $row['name']; ?></td>

                <td><?php echo $row['room_title']; ?></td>

                <td class="<?php echo $row['status']; ?>">
                    <?php echo ucfirst($row['status']); ?>
                </td>

            </tr>

            <?php } ?>

        </table>

    </div>

</div>

<!-- CHART -->
<script>

const ctx = document.getElementById('chart');

new Chart(ctx, {

    type:'bar',

    data:{
        labels:['Rooms','Bookings','Pending','Accepted','Messages'],

        datasets:[{
            label:'Owner Analytics',

            data:[
                <?php echo $rooms; ?>,
                <?php echo $bookings; ?>,
                <?php echo $pending; ?>,
                <?php echo $accepted; ?>,
                <?php echo $messages; ?>
            ],

            borderWidth:1
        }]
    },

    options:{
        responsive:true
    }

});

/* DARK MODE */
function toggleDark(){
    document.body.classList.toggle("dark");
}

</script>

</body>
</html>